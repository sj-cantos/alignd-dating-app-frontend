      ...
      {!isEditing ? (
        ...
        <div className="flex gap-3">
          ...
          <Button
            onClick={handleSave}
            disabled={saving}
            className="bg-gradient-to-r from-success to-emerald-500 text-success-foreground border-brutal border-border shadow-brutal-lg font-black px-5 py-2"
          >
          ...
          <section className="space-y-5">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (/^\d*$/.test(value)) {
                      const numValue = value === '' ? 18 : parseInt(value) || 18;
                      setFormData((p) => ({ ...p, age: numValue }));
                    }
                  }}
                  onBlur={() => {
                    const n = parseInt(formData.age as any);
                    if (isNaN(n) || n < 18) setFormData((p) => ({ ...p, age: 18 }));
                    else if (n > 100) setFormData((p) => ({ ...p, age: 100 }));
                    else setFormData((p) => ({ ...p, age: n }));
                  }}
                  disabled={!isEditing}
                  className="border-brutal border-border font-medium bg-background shadow-brutal-sm focus:shadow-brutal"
                />
              ...
              <div className="flex flex-wrap gap-3">
                {Object.values(Gender).map((g) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full border-2 border-border font-bold transition-all duration-150 
                      ${formData.gender === g
                        ? 'bg-gradient-to-r from-primary to-pink-500 text-primary-foreground shadow-brutal'
            ...
            <div>
              ...
              <div className="mt-4 flex flex-col sm:flex-row items-center sm:items-start gap-5">
                <div className="w-28 h-28 rounded-2xl border-brutal border-border shadow-brutal bg-muted overflow-hidden">
                  {selectedFile ? (
                    <img
                ...
                <div className="flex flex-col gap-3 w-full">
                  <Input
                    ...
                    className="border-brutal border-border font-medium bg-background shadow-brutal-sm focus:shadow-brutal file:border-0 file:bg-gradient-to-r file:from-primary file:to-pink-500 file:text-primary-foreground file:font-bold"
                  />
                  {selectedFile && (
                    <Button
                      type="button"
                      ...
                      className="bg-gradient-to-r from-success to-emerald-500 hover:opacity-90 text-success-foreground border-brutal border-border font-bold shadow-brutal-sm"
                    >
          ...
          <section className="space-y-4">
            ...
            {isEditing && (
              <div className="flex gap-2">
                ...
                <Button
                  onClick={addInterest}
                  className="bg-gradient-to-r from-success to-emerald-500 text-success-foreground border-brutal border-border font-bold shadow-brutal-sm"
                >
                  Add
                </Button>
              </div>
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-full border-2 border-border font-bold transition-all duration-150 
                      ${formData.interestedInGender.includes(g)
                        ? 'bg-gradient-to-r from-success to-emerald-500 text-success-foreground shadow-brutal'
                        : 'bg-background text-foreground hover:bg-muted'
                      } ${!isEditing ? 'opacity-60 cursor-not-allowed' : ''}`}
                  >
                    {g}
